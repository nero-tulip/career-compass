...
export default function MacroPage() {
  ...
  const onSelect = (questionId: string, score: number) => {
    setAnswers(prev => {
      ...
      if (i >= 0) { const next = [...prev]; next[i] = { ...next[i], score }; return next; }
  ...
  const onNext = async () => {
    ...
    router.push(`/riasec?rid=${rid}`);
  };

  return (
    <div className="max-w-3xl mx-auto py-10 px-4">
      <div className="mb-6">
        <ProgressBar value={all.length ? answers.length / all.length : 0} label="Part 1 of 2" />
      </div>
